...
const SuperAdminComplaints = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const resolvers = allUsers.filter(u => ['Team Leader', 'Manager', 'Staff'].includes(u.role));
      setTeamLeaders(resolvers);
      
      setDepartments(deptRes.data || []);
    } catch (err) {
      console.error('Failed to load Super Admin complaints:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveManage = async (e) => {
    ...
    try {
      const payload = {
        ...
        comment: statusComment || `Super Admin updated status to ${selectedStatus}`
      };
  ...
  const handleAddAdminComment = async (e) => {
    ...
    try {
      const res = await API.put(`/complaints/${timelineModalComplaint._id}`, {
        comment: `[Super Admin Note]: ${adminComment.trim()}`,
                      ...
                      <tr key={c._id} style={{ borderBottom: '1px solid #F1F5F9', background: isEscalated ? '#FFFDFD' : 'transparent' }} onMouseEnter={e => e.currentTarget.style.background='#F8FAFC'} onMouseLeave={e => e.currentTarget.style.background= isEscalated ? '#FFFDFD' : 'transparent'}>
                        ...
                        <td style={{ padding: '1.15rem 1.25rem', minWidth: '160px' }}>
                          <span style={{ 
                            ...
                            {c.status === 'Escalated' && <ShieldAlert size={11} />}
                            {c.status}
                          </span>
                          {c.feedbackRating && (
                        ...
                        <td style={{ padding: '1.15rem 1.25rem', textAlign: 'right', minWidth: '220px' }}>
                          <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'flex-end', alignItems: 'center' }}>
                            ...
                            <button 
                              ...
                              style={{ background: '#2563EB', color: '#FFFFFF', border: 'none', padding: '0.42rem 0.7rem', borderRadius: '8px', fontSize: '0.75rem', fontWeight: '700', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.25rem', boxShadow: '0 2px 4px rgba(37,99,235,0.2)' }}
                            >
      ...
      {timelineModalComplaint && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.65)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(5px)', padding: '1rem' }}>
          <div style={{ background: '#FFFFFF', borderRadius: '22px', maxWidth: '680px', width: '100%', maxHeight: '88vh', overflow: 'hidden', boxShadow: '0 30px 60px -10px rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column' }}>
            ...
            {/* ADD SUPER ADMIN OVERSIGHT COMMENT */}
            <form onSubmit={handleAddAdminComment} style={{ padding: '1rem 1.75rem', borderTop: '1px solid #F1F5F9', background: '#FAFAFA', display: 'flex', gap: '0.6rem' }}>
              <input 
                type="text" 
                value={adminComment} 
                onChange={e => setAdminComment(e.target.value)} 
                placeholder="Post executive audit note / directive to ticket thread..." 
